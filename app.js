...
function showFirebaseError(err) {
  console.error(err);
  toast("Firebase не дает доступ. Проверь Firestore Rules: users/public должен быть открыт.");
}
...
document.addEventListener("submit", async (e) => {
  e.preventDefault();
  try {
    if (e.target.id === "deliveryForm") await saveDelivery(e.target);
    if (e.target.id === "supplierForm") await saveSupplier(e.target);
    if (e.target.id === "productForm") await saveProduct(e.target);
  } catch (err) {
