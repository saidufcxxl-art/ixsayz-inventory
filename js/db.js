...
import { addDoc, collection, deleteDoc, doc, getDocs, onSnapshot, serverTimestamp, setDoc, updateDoc, writeBatch } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js";
...
export function listenCollection(uid, name, cb, onError = console.error) {
  return onSnapshot(paths[name](uid), (snap) => cb(snap.docs.map((item) => ({ id: item.id, ...item.data() }))), onError);
}
export async function createItem(uid, name, data) {
  const ref = doc(paths[name](uid));
  await setDoc(ref, withMeta(uid, data, ref.id));
  return ref.id;
}
